...
def rewrite_star(text: str) -> str:
    ...
    has_quantified_result = (
        "%" in t
        or "reduc" in t.lower()
        or "improv" in t.lower()
        or "increas" in t.lower()
        or "accelerat" in t.lower()
        or "cut" in t.lower()
        or "enabling" in t.lower()
        or re.search(r"\d+[xX]", t)  # e.g., "2x faster"
        or re.search(r"\d+\s*(hour|day|week|month|year)", t, re.IGNORECASE)
        or "from" in t.lower()
        and "to" in t.lower()  # e.g., "from days to hours"
    )
