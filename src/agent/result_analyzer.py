...
class ResultAnalyzer:
    ...
    def _determine_status(
        self, output: str, error: str, return_code_success: bool
    ) -> str:
        """
    ...
    def _extract_information(self, output: str, error: str) -> Dict[str, Any]:
        ...
        count_matches = re.findall(
            r"(\d+)\s+(?:resume|item|entry|entries|file)", combined, re.IGNORECASE
        )
        if count_matches:
    ...
    def _format_extracted_info(self, extracted_info: Dict[str, Any]) -> str:
        ...
        if "counts" in extracted_info and extracted_info["counts"]:
            lines.append(
                f"[INFO]    Items processed: {', '.join(map(str, extracted_info['counts']))}"
            )

        return "\n".join(lines) if lines else ""

    def _generate_suggestions(
        self, command: str, status: str, extracted_info: Dict[str, Any]
    ) -> List[str]:
        """
