...
class NLCommandParser:
    ...
    def parse(self, command: str, default_resume: str = "Master Resume") -> Dict[str, Any]:
    ...
    def _extract_resume_params(self, intent: str) -> Dict[str, Any]:
        ...
        if intent == 'duplicate':
            ...
            pattern = r'duplicate\s+(?:the\s+)?(.+?)\s+(?:resume\s+)?(?:as|to|into)\s+(.+)'
            match = re.search(pattern, self.command_text)
            if match:
    ...
    def _generate_crud_command(self, intent: str, entity: str, resume: str, params: Dict[str, Any]) -> str:
