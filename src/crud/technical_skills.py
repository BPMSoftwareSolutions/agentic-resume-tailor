...
def add_category(
    resume_data: Dict[str, Any], category: str, skills: str
) -> Dict[str, Any]:
    """
    ...
    if category_lower in resume_data["technical_proficiencies"]:
        ...
        print_info(
            "Use --update-category to replace or --append-to-category to add skills"
        )
        sys.exit(3)
...
def append_to_category(
    ...
    resume_data["technical_proficiencies"][category_lower] = (
        current_skills + skills.strip()
    ).strip()
    return resume_data
...
Examples:
  ...
  python src/crud/technical_skills.py --resume "Ford" --show "cloud"
    ...
    parser.add_argument(
        "--delete-category", metavar="CATEGORY", help="Delete skills category"
    )
    parser.add_argument(
        "--list", action="store_true", help="List all skills categories"
    )
    parser.add_argument(
        "--show", metavar="CATEGORY", help="Show specific skills category"
    )
