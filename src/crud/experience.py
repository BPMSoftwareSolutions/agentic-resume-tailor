...
def find_experience_by_employer(
    resume_data: Dict[str, Any], employer: str
) -> Optional[int]:
    """
...
def delete_bullet(
    resume_data: Dict[str, Any], employer: str, index: int
) -> Dict[str, Any]:
    """
    ...
    for i, exp in enumerate(experience_list):
        ...
        for j, bullet in enumerate(bullets):
            ...
            if verbose:
                ...
                print(
                    f"        [{j}] {wrapped[8:]}"
                )  # Remove initial indent from first line since we add it
            else:
...
Examples:
  ...
  python src/crud/experience.py --resume "Ford" --from-markdown "experience.md"
    ...
    parser.add_argument(
        "--add-bullet", action="store_true", help="Add bullet to experience"
    )
    ...
    parser.add_argument(
        "--from-markdown", metavar="FILE", help="Import from markdown file"
    )
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Replace all experience (for --from-markdown)",
    )
    ...
    parser.add_argument(
        ...
        "-v",
        action="store_true",
        help="Show full bullet text (for --list)",
    )
    ...
    try:
        ...
        if args.add:
            ...
            resume_data = add_experience(
                resume_data, args.employer, args.role, args.dates, args.location or ""
            )
            modified = True
        ...
        if args.update_bullet:
            ...
            resume_data = update_bullet(
                resume_data, args.employer, args.index, args.text, tags
            )
            modified = True
        ...
        if args.from_markdown:
            ...
            modified = True
            print_success(
                f"Imported experience from {md_file} ({'replaced' if args.replace else 'prepended'})"
            )
