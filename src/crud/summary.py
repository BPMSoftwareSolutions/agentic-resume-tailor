...
Examples:
  ...
  python src/crud/summary.py --resume "Ford" --show
    ...
    # Operations
    parser.add_argument(
        "--update", metavar="TEXT", help="Update/replace entire summary"
    )
    parser.add_argument(
        "--append", metavar="TEXT", help="Append text to existing summary"
    )
    parser.add_argument(
        "--from-file", metavar="FILE", help="Load summary from text file"
    )
    parser.add_argument("--show", action="store_true", help="Show current summary")
    ...
    if not any(operations):
        parser.error(
            "At least one operation (--update, --append, --from-file, --show) must be specified"
        )
    ...
    try:
        ...
        if args.from_file:
            ...
            modified = True
            print_success(
                f"Loaded summary from file: {file_path} ({len(summary_text)} characters)"
            )
