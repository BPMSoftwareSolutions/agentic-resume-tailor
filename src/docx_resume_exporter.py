...
class DOCXResumeExporter:
    ...
    def _check_available_methods(self) -> dict:
        ...
        methods = {"python-docx": False, "pandoc": False, "weasyprint": False}
        ...
        try:
            ...
            if header:
                ...
                if contact_section:
                    ...
                    # Location
                    location_elem = contact_section.find(
                        attrs={"data-field": "location"}
                    )
                    if location_elem:
            ...
            for section in sections:
                ...
                if section_type == "summary" or "summary" in section.get("class", []):
                ...
                elif section_type == "experience" or "experience" in section.get(
                    "class", []
                ):
                    # Experience items
                    ...
                    if certs_section:
                        ...
                        for cert in cert_items:
                            doc.add_paragraph(
                                cert.get_text().strip(), style="List Bullet"
                            )
                    ...
                    if two_column:
                        ...
                        if len(skills_columns) == 2:
                            ...
                            for col_idx, column in enumerate(skills_columns):
                                ...
                                subsection_heading = column.find(
                                    class_="subsection-heading"
                                )
                                if subsection_heading:
                                    p = cell.add_paragraph(
                                        subsection_heading.get_text().strip()
                                    )
                                    run = p.runs[0]
                                    run.font.size = Pt(14)
                                    run.font.bold = True
                                ...
                                skill_categories = column.find_all(
                                    class_="skill-category"
                                )
                                ...
                                if expertise_list:
                                    expertise_items = expertise_list.find_all(
                                        class_="expertise-item"
                                    )
                                ...
                                if (
                                    len(cell.paragraphs) > 0
                                    and not cell.paragraphs[0].text.strip()
                                ):
                    ...
                    for p_elem in section.find_all(
                        ["p", "div"], class_=lambda c: c and "text" in c
                    ):
    ...
    def _try_pdf_conversion_method(self, html_path: str, output_docx_path: str) -> bool:
        ...
        try:
            ...
            print(
                f"  INFO: Note: PDF generated, but DOCX conversion requires additional tools"
            )
            print(
                f"  INFO: Consider using pandoc or python-docx for direct DOCX export"
            )
