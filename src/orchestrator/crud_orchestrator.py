...
class CRUDOrchestrator:
    ...
    def generate_operations(self, job_data: Dict[str, Any], match_result: Dict[str, Any], 
                          resume_name: str, intent: str = "tailor") -> List[Dict[str, Any]]:
        """
        ...
        if responsibilities:
            ...
            expertise_areas = self._extract_expertise_from_responsibilities(responsibilities)
    ...
    def _categorize_skills(self, skills: List[str]) -> Dict[str, List[str]]:
        ...
        language_keywords = ['python', 'java', 'javascript', 'typescript', 'c++', 'c#', 'go', 'rust', 'ruby', 'php']
        cloud_keywords = ['aws', 'azure', 'gcp', 'cloud platform', 'google cloud']
        database_keywords = ['sql', 'mysql', 'postgresql', 'mongodb', 'redis', 'dynamodb']
        devops_keywords = ['docker', 'kubernetes', 'jenkins', 'ci/cd', 'terraform', 'ansible', 'git', 'datadog']
        billing_keywords = ['zuora', 'stripe', 'chargebee', 'subscription billing', 'revpro']
    ...
    def _generate_summary_hint(self, job_data: Dict[str, Any], relevant_exp: List[Dict]) -> str:
    ...
    def _extract_expertise_from_responsibilities(self, responsibilities: List[str]) -> List[str]:
        """Extract expertise areas from job responsibilities"""
        expertise = []
        
        # Common expertise patterns
        patterns = [
            'leadership', 'architecture', 'microservices', 'cloud', 'devops',
            'billing', 'subscription', 'revenue', 'compliance', 'sox'
        ]
        
        for resp in responsibilities:
            resp_lower = resp.lower()
            for pattern in patterns:
                if pattern in resp_lower and pattern not in [e.lower() for e in expertise]:
                    # Capitalize properly
                    if pattern == 'sox':
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: python crud_orchestrator.py <job_posting_file> <resume_file> [resume_name]")
        sys.exit(1)
