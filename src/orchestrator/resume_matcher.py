...
class ResumeMatcher:
    ...
    def match(
        self, job_data: Dict[str, Any], resume_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        score = self._calculate_match_score(
            matching_skills, required_skills, relevant_experience, job_data
        )

        # Generate suggestions
        suggestions = self._generate_suggestions(
            missing_skills, relevant_experience, job_data
        )
        ...
        return {
            ...
            "details": {
                ...
                "skill_match_percentage": (
                    round(len(matching_skills) / len(required_skills) * 100, 2)
                    if required_skills
                    else 0
                ),
    ...
    def _find_relevant_experience(
        ...
        for exp in experience:
            ...
            for bullet in bullets:
                ...
                skill_matches = sum(
                    1 for skill in required_skills if skill in bullet_lower
                )
                if skill_matches > 0:
                    relevance_score += skill_matches
                    matching_bullets.append(
                        {"text": bullet, "skill_matches": skill_matches}
                    )
                ...
                resp_matches = sum(
                    ...
                    if any(word in bullet_lower for word in resp.split()[:5])
                )
                if resp_matches > 0:
    ...
    def _calculate_match_score(
        ...
        skill_score = (
            (len(matching_skills) / len(required_skills) * 50) if required_skills else 0
        )
    ...
    def _generate_suggestions(
        ...
        if relevant_experience:
            ...
            suggestions.append(
                f"Emphasize experience at {top_exp['employer']} - it has {len(top_exp['matching_bullets'])} relevant bullets"
            )
        ...
        if responsibilities:
            suggestions.append(
                f"Align experience bullets with key responsibilities: {responsibilities[0][:100]}..."
            )
...
def main():
    ...
    if result["relevant_experience"]:
        ...
        for exp in result["relevant_experience"][:3]:
            ...
            print(
                f"    Relevance: {exp['relevance_score']:.1f}, Matching bullets: {len(exp['matching_bullets'])}"
            )
