...
class TestVisualAppearanceValidation(unittest.TestCase):
    ...
    def test_table_cell_padding_and_spacing(self):
                    ...
                    if cell_text:
                        ...
                        if len(cell.paragraphs) == 1 and len(cell_text) > 200:
                            ...
                            has_spacing = (
                                ...
                                or para.paragraph_format.space_after
                                or para.paragraph_format.line_spacing
                            )
        ...
        if appearance_issues:
            ...
            if len(appearance_issues) > 5:
                appearance_issues = appearance_issues[:5] + [
                    f"... and {len(appearance_issues) - 5} more issues"
                ]
            self.fail(
                f"Table visual appearance issues:\n" + "\n".join(appearance_issues)
            )
    ...
    def test_font_rendering_consistency(self):
        ...
        if (
            undefined_fonts > 10
        ):  # Some undefined fonts are normal, but too many indicate issues
            font_issues.append(
                f"Many undefined fonts ({undefined_fonts}) - may cause inconsistent rendering"
            )
        ...
        if len(non_professional_fonts) > 0:
            font_issues.append(
                f"Non-standard fonts used: {list(non_professional_fonts)} - may not render consistently"
            )
                    ...
                    for paragraph in cell.paragraphs:
                        for run in paragraph.runs:
                            ...
                            if run.bold and run.italic and run.underline:
                                inconsistent_styling.append(
                                    "Text with bold+italic+underline (may be over-formatted)"
                                )

        # Too many colors can look unprofessional
        if len(colors_used) > 4:
            styling_issues.append(
                f"Too many text colors used ({len(colors_used)}) - may look unprofessional"
            )

        # Check for styling consistency
        if len(inconsistent_styling) > 3:
            styling_issues.append(
                f"Over-formatted text detected in {len(inconsistent_styling)} places"
            )
        ...
        for table_idx, table in enumerate(self.output_doc.tables):
            ...
            if len(table.rows) > 0 and len(table.columns) > 0:
                ...
                if empty_ratio > 0.6 and not is_header_table:  # More than 60% empty
                    table_issues.append(
                        f"Table {table_idx}: Mostly empty ({empty_ratio:.1%}) - may look sparse"
                    )
                ...
                if cell_lengths:
                    ...
                    if non_empty_lengths and max_length > 0:
                        ...
                        is_label_value_table = (
                            len(table.columns) == 2 and len(table.rows) >= 4
                        )
                        if (
                            max_length / min(non_empty_lengths) > 10
                            and not is_label_value_table
                        ):
                            table_issues.append(
                                f"Table {table_idx}: Very uneven content distribution"
                            )
    ...
    def test_prevents_document_corruption_indicators(self):
        ...
        if total_tables > 0 and total_content > 0:
            ...
            if content_per_table < 50:  # Very little content per table
                corruption_indicators.append(
                    f"Very little content per table ({content_per_table:.1f} chars)"
                )

            if content_per_table > 2000:  # Too much content per table
                corruption_indicators.append(
                    f"Excessive content per table ({content_per_table:.1f} chars)"
                )
        ...
        if len(all_cell_texts) > 0:
            ...
            if duplicate_ratio > 0.3:  # More than 30% duplicates
                corruption_indicators.append(
                    f"High content duplication ({duplicate_ratio:.1%}) - may indicate generation errors"
                )

        if corruption_indicators:
            self.fail(
                f"Document corruption indicators:\n" + "\n".join(corruption_indicators)
            )
    ...
    def test_compare_visual_complexity_with_reference(self):
        ...
        complexity_diff = abs(
            output_complexity["total_score"] - reference_complexity["total_score"]
        )
        ...
        if (
            output_complexity["table_complexity"]
            < reference_complexity["table_complexity"] * 0.7
        ):
            complexity_issues.append("Tables significantly less complex than reference")

        if (
            output_complexity["formatting_complexity"]
            < reference_complexity["formatting_complexity"] * 0.7
        ):
            complexity_issues.append(
                "Formatting significantly less complex than reference"
            )
    ...
    def _calculate_visual_complexity(self, doc):
        ...
        complexity["content_variety"] = (
            sum(1 for indicator in content_indicators if indicator in all_text) * 3
        )
