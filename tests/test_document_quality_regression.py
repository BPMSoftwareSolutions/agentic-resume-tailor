...
class TestDocumentQualityRegression(unittest.TestCase):
    ...
    def test_prevents_cramped_layout_regression(self):
            ...
            for row in table.rows:
                for cell in row.cells:
                    ...
                    if para_count > 0 and text_length > 100:  # Substantial content
                        ...
                        if (
                            avg_text_per_para > 200
                        ):  # Very long paragraphs might indicate cramped formatting
                            cell_spacing_issues += 1

            if (
                cell_spacing_issues > len(table.rows) * 0.5
            ):  # More than half the rows have issues
                cramped_tables += 1

        if cramped_tables > 2:
            quality_issues.append(
                f"Detected {cramped_tables} tables with cramped formatting"
            )
        ...
        if total_paragraphs > 0 and total_text > 0:
            ...
            if avg_para_length > 300:  # Very long average paragraphs
                quality_issues.append(
                    f"Average paragraph length too long ({avg_para_length:.0f} chars) - may indicate cramped layout"
                )
    ...
    def test_prevents_inconsistent_formatting_regression(self):
        ...
        if len(fonts_used) > 4:
            formatting_issues.append(
                f"Too many fonts used ({len(fonts_used)}): {list(fonts_used)}"
            )

        if len(font_sizes_used) > 6:
            formatting_issues.append(
                f"Too many font sizes used ({len(font_sizes_used)}): {sorted(font_sizes_used)}"
            )
        ...
        if font_sizes_used:
            ...
            if max_size / min_size > 3:  # More than 3x difference
                formatting_issues.append(
                    f"Extreme font size variation: {min_size}pt to {max_size}pt"
                )

        if formatting_issues:
            self.fail(
                f"Formatting consistency issues detected:\n"
    ...
    def test_prevents_poor_visual_hierarchy_regression(self):
        ...
        if not has_size_variation and not has_bold_headers:
            hierarchy_issues.append(
                "No clear visual hierarchy detected (no size variation or bold headers)"
            )
        ...
        found_sections = [
            section for section in expected_sections if section in all_text
        ]

        if len(found_sections) < 2:
            hierarchy_issues.append(
                f"Poor content organization - only found sections: {found_sections}"
            )

        if hierarchy_issues:
            self.fail(
                f"Visual hierarchy issues detected:\n" + "\n".join(hierarchy_issues)
            )
    ...
    def test_prevents_unprofessional_spacing_regression(self):
        ...
        if len(self.output_doc.tables) > 0:
            ...
            for table_idx, table in enumerate(self.output_doc.tables):
                ...
                if (
                    total_cells > 0 and empty_cells / total_cells > 0.7
                ):  # More than 70% empty
                    spacing_issues.append(
                        f"Table {table_idx} is mostly empty ({empty_cells}/{total_cells} cells)"
                    )
    ...
    def test_document_readability_standards(self):
        ...
        if word_count < 100:
            readability_issues.append(
                f"Document too short ({word_count} words) - may be incomplete"
            )

        if word_count > 2000:
            readability_issues.append(
                f"Document very long ({word_count} words) - may need better formatting"
            )
        ...
        if sentence_count > 0:
            ...
            if avg_words_per_sentence > 40:
                readability_issues.append(
                    f"Very long sentences (avg {avg_words_per_sentence:.1f} words) - hard to read"
                )
        ...
        if all_text:
            ...
            if capital_ratio > 0.15:  # More than 15% capitals
                readability_issues.append(
                    f"Excessive capitalization ({capital_ratio:.1%}) - may indicate formatting issues"
                )
            if capital_ratio < 0.02:  # Less than 2% capitals
                readability_issues.append(
                    f"Insufficient capitalization ({capital_ratio:.1%}) - may indicate formatting issues"
                )
    ...
    def test_professional_quality_benchmarks(self):
        ...
        table_diff = abs(
            output_metrics["table_count"] - reference_metrics["table_count"]
        )
        if table_diff > 2:
            quality_issues.append(
                f"Table count significantly different: {output_metrics['table_count']} vs {reference_metrics['table_count']}"
            )

        # Content density should be reasonable
        content_density_diff = abs(
            output_metrics["content_density"] - reference_metrics["content_density"]
        )
        if content_density_diff > 50:  # More than 50 chars/para difference
            quality_issues.append(
                f"Content density very different: {output_metrics['content_density']:.1f} vs {reference_metrics['content_density']:.1f}"
            )

        # Should have some formatting variation
        if not output_metrics["has_formatting_variety"]:
            quality_issues.append(
                "No formatting variety detected - document may look plain"
            )

        # Should have reasonable structure complexity
        if output_metrics["structure_complexity"] < 3:
            quality_issues.append(
                f"Low structure complexity ({output_metrics['structure_complexity']}) - document may look simple"
            )

        if quality_issues:
            self.fail(
                f"Professional quality issues detected:\n" + "\n".join(quality_issues)
            )
    ...
    def _calculate_quality_metrics(self, doc):
        ...
        if metrics["paragraph_count"] > 0:
            metrics["content_density"] = (
                metrics["total_chars"] / metrics["paragraph_count"]
            )
        else:
            metrics["content_density"] = 0

        metrics["has_formatting_variety"] = (
            len(metrics["font_variety"]) > 1
            or len(metrics["size_variety"]) > 1
            or metrics["has_bold"]
        )
