...
class TestContentAccuracyValidation(unittest.TestCase):
            ...
            for match in matches:
                ...
                if '      ' in found_text:  # Multiple spaces
                    ...
                    if not self._is_intentional_alignment(found_text):
                        formatting_issues.append(f"Excessive spacing in: '{found_text}'")
        ...
        for i, line in enumerate(lines):
            ...
            if 'CGI - Daugherty' in line and '2022' in line:
                ...
                if i + 1 < len(lines):
                    ...
                    if 'Principle Consultant' in next_line or 'Principal Consultant' in next_line:
                        cgi_title_on_separate_line = True
        
        if cgi_section_found and not cgi_title_on_separate_line:
            structure_issues.append("CGI section job title should be on separate line from company/dates")
        
        # Check for other structural consistency
        company_lines = []
        for line in lines:
            line = line.strip()
            if any(company in line for company in ['BPM Software Solutions', 'Interactive Business Solutions', 'Soave Enterprises', 'John Deere Landscapes']):
        ...
        if len(company_lines) >= 2:
            ...
            for i, line in enumerate(company_lines[1:], 1):
                ...
                if line_structure != first_structure:
                    structure_issues.append(f"Inconsistent structure: '{company_lines[0]}' vs '{line}'")
        
        if structure_issues:
            self.fail(f"Job title section structure issues:\n" + "\n".join(structure_issues))
    ...
    def test_certification_formatting_consistency(self):
        ...
        for cert in expected_certifications:
            if cert not in all_text:
                ...
                if len(cert_words) > 1:
                    ...
                    if partial_match:
                        formatting_issues.append(f"Partial match for certification: '{cert}'")
                    else:
        ...
        if combined_cert_lines:
            formatting_issues.append(f"Certifications combined on single lines: {combined_cert_lines}")
    ...
    def test_whitespace_and_spacing_consistency(self):
        ...
        for i, line in enumerate(lines):
            if line.strip():  # Skip empty lines
                ...
                if '       ' in line:  # 7+ spaces
                    spacing_issues.append(f"Line {i+1}: Excessive spacing - '{line[:50]}...'")
                
                # Check for mixed tab/space patterns
                if '\t' in line and '  ' in line:
                    spacing_issues.append(f"Line {i+1}: Mixed tabs and spaces - '{line[:50]}...'")
        ...
        if len(spacing_issues) > 10:
            spacing_issues = spacing_issues[:10] + [f"... and {len(spacing_issues) - 10} more spacing issues"]
    ...
    def _analyze_line_structure(self, line):
        ...
        structure = {
            ...
            'spacing_pattern': 'tabs' if '\t' in line else 'spaces' if '    ' in line else 'minimal'
        }
        return structure
