...
class TestVisualFormattingValidation(unittest.TestCase):
    ...
    def test_table_cell_alignment_consistency(self):
            ...
            for row_idx, row in enumerate(table.rows):
                for cell_idx, cell in enumerate(row.cells):
                    for para_idx, paragraph in enumerate(cell.paragraphs):
                        ...
                        if paragraph.alignment not in [WD_ALIGN_PARAGRAPH.LEFT, WD_ALIGN_PARAGRAPH.CENTER, None]:
        ...
        if len(alignment_issues) > 5:
            self.fail(f"Many alignment inconsistencies found:\n" + "\n".join(alignment_issues[:10]))
    ...
    def test_spacing_and_margins_reasonable(self):
        ...
        for section in self.output_doc.sections:
            ...
            if section.top_margin.inches < 0.3:
                margin_issues.append(f"Top margin too small: {section.top_margin.inches:.2f} inches")
            if section.top_margin.inches > 2.0:
                margin_issues.append(f"Top margin too large: {section.top_margin.inches:.2f} inches")
            
            if section.bottom_margin.inches < 0.3:
                margin_issues.append(f"Bottom margin too small: {section.bottom_margin.inches:.2f} inches")
            if section.bottom_margin.inches > 2.0:
                margin_issues.append(f"Bottom margin too large: {section.bottom_margin.inches:.2f} inches")
            
            if section.left_margin.inches < 0.3:
                margin_issues.append(f"Left margin too small: {section.left_margin.inches:.2f} inches")
            if section.left_margin.inches > 2.0:
                margin_issues.append(f"Left margin too large: {section.left_margin.inches:.2f} inches")
            
            if section.right_margin.inches < 0.3:
                margin_issues.append(f"Right margin too small: {section.right_margin.inches:.2f} inches")
            if section.right_margin.inches > 2.0:
                margin_issues.append(f"Right margin too large: {section.right_margin.inches:.2f} inches")
    ...
    def test_text_formatting_quality(self):
        ...
        if len(font_names) > 5:
            formatting_issues.append(f"Too many different fonts used: {list(font_names)}")
        ...
        if paragraph_count > 0:
            ...
            if avg_para_length > 500:
                formatting_issues.append(f"Average paragraph length too long: {avg_para_length:.1f} characters")
    ...
    def test_professional_appearance_indicators(self):
        ...
        content_keywords = ['experience', 'education', 'skills', 'technical', 'proficiencies']
        all_text = ""
        for table in self.output_doc.tables:
            for row in table.rows:
                for cell in row.cells:
        ...
        missing_indicators = [key for key, value in professional_indicators.items() if not value]
    ...
    def test_compare_with_reference_structure(self):
        ...
        content_diff_ratio = abs(output_stats['total_text_length'] - reference_stats['total_text_length']) / reference_stats['total_text_length']
        ...
        if structure_issues:
            self.fail(f"Structural differences from reference:\n" + "\n".join(structure_issues))
