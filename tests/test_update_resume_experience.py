...
from update_resume_experience import (find_resume_by_identifier,
                                      load_resume_index,
        ...
        "experience": [
            {
                ...
                "bullets": [{"text": "Old bullet 1", "tags": []}],
            }
        ],
    }
...
class TestFindResumeByIdentifier:
    ...
    def test_find_by_full_name(self, temp_data_dir):
        ...
        resume = find_resume_by_identifier(
            index_data, "Sidney_Jones_Senior_Software_Engineer_GM"
        )
        assert resume is not None
...
class TestUpdateResumeExperience:
    ...
    def test_prepend_experience(self, temp_data_dir):
        ...
        update_resume_experience(
            temp_data_dir["data_dir"], "test-resume-1", experiences, replace=False
        )
    ...
    def test_replace_experience(self, temp_data_dir):
        ...
        update_resume_experience(
            temp_data_dir["data_dir"], "test-resume-1", experiences, replace=True
        )
    ...
    def test_update_nonexistent_resume(self, temp_data_dir):
        ...
        with pytest.raises(FileNotFoundError):
            update_resume_experience(
                temp_data_dir["data_dir"], "nonexistent-id", experiences, replace=False
            )
